...
./autogen.sh
./configure --prefix="$WORK" --disable-shared --without-brotli --without-bzip2 PKG_CONFIG_PATH="$WORK/lib/pkgconfig"
make -j$(nproc) clean
make -j$(nproc)
make install
...
$CXX $CXXFLAGS -std=c++11 -I"$WORK/poppler/cpp" -I"$TARGET/repo/cpp" \
    ...
    "$WORK/lib/libfreetype.a" $LDFLAGS $LIB_FUZZING_ENGINE $LIBS -ljpeg -lz \
